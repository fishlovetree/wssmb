...
function PhotoSphereViewer(options) {
  ...
  this.defaultZoom = options.zoom ? options.zoom : 10;
  ...
  this.zoom(this.defaultZoom, false);
...
PhotoSphereViewer.prototype._onMouseWheel = function(evt) {
  ...
  if (delta !== 0) {
	  var zoomVal = delta * this.config.mousewheel_factor;
	  var isZoom = true;
	  if(zoomVal < 0 && this.prop.zoom_lvl > 90){
		  isZoom = false; 
	  }
	  if(zoomVal > 0 && this.prop.zoom_lvl < 71){
		  isZoom = false;
	  }
	  if(isZoom){
		  this.zoom(this.prop.zoom_lvl - zoomVal);
	  }
  }
};
