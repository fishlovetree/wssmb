...
<div id="dlg" class="easyui-dialog" style="width:320px;height:250px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
    <div id="divFm">
	    <form id="fm" class="easyui-form" method="post" >
	        <input type="hidden" id="roletype" name="roletype" value="0"/> 
...
<script type="text/javascript">	
	$(function(){
	    $('#dg').datagrid({
			...
			fit: true,   //自适应大小
			toolbar:'#toolbar',
			url:'${pageContext.request.contextPath}/sysRole/roleDataGrid?roletype=0',
			columns: [[  
...
function saveRole(){
	$('#fm').form('submit',{
		...
		success: function(result){	
			if (result == "repeat"){
				$.messager.alert('提示','角色名称已存在。','warning');
			}
...
function leftUser(id){ 
	$('#left_ul li').remove();
	var mData={"roleid":id, "roletype":0};
	 $.ajax({
          type : 'POST',
