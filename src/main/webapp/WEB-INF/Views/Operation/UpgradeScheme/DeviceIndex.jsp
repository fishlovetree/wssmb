...
$(function(){
	$('#dg').datagrid({
		...
		pageSize:10,
		fit: true,   //自适应大小
		url:'${pageContext.request.contextPath}/upgradeScheme/deviceSchemeDataGrid',
		toolbar: '#toolbar',
		columns: [[         	
...
//编制升级方案
function addScheme(){
	var href='${pageContext.request.contextPath}/upgradeScheme/devicePreparation';
    parent.addTab(href, '设备升级方案编制', '');
}
...
function deleteScheme(){
	...
	if (row){
		$.messager.confirm('删除','确定要删除该设备升级方案吗？',function(r){
			if (r){
				$.ajax({
				    type:'POST', 
			        url:'${pageContext.request.contextPath}/upgradeScheme/deleteDeviceScheme',           
			        data:{"id": row.id},        
				    success:function(d){
					    if (d == "success") {
...
function showDetails(){
	...
	if (row){
		...
		$('#detailTbl').datagrid({
			...
			pagination:true,
			pageSize:10,
			url:'${pageContext.request.contextPath}/upgradeScheme/deviceSchemeDetailsDataGrid?id='+row.id,
			columns: [[         	
			    {title: '系统类型代码', field: 'systemtype',width:'80'}, 
			    {title: '设备类型代码', field: 'equipmenttype', width: '80'},
...
function exportScheme(){
	...
	if (row){
	    ...
	    form.attr('action', '${pageContext.request.contextPath}/upgradeScheme/exportDevice');
