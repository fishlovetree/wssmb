		    ...
		    <div class="easyui-layout" fit="true">
		        <div id="mainPanel" region="north" style="width:100%;height:90px;position:relative;overflow-y:hidden;">
		        	<table border="0" cellspacing="8" cellpadding="8">
                        <tr>
                            <td>
                                <input class="easyui-textbox" type="text" style="width:200px" id='unitaddr' name="unitaddr" data-options="label:'终端地址'"/>
                            </td>
                            <td>
                                <input class="easyui-textbox" type="text" style="width:200px" id='unitname' name="'unitname'" data-options="label:'终端名称'"/>
                            </td>
                            <td>
                                <input class="easyui-textbox" type="text" style="width:200px" id='oldver' name="oldver" data-options="label:'当前版本号'"/>
                            </td>
                            <td>
                                <a href="javascript:void(0)" class="easyui-linkbutton" onclick="searchUnit()" title="检索">检索</a>
                            </td>
                        </tr>
                        <tr>
                            <td>
...
$(function(){
	$('#unitTbl').datagrid({
		url: '${pageContext.request.contextPath}/upgradeScheme/unitDataGrid?Math.random()',
		singleSelect:false,
		...
		columns: [[         	
            ...
            {title: '终端名称', field: 'unitname', width:'180px'},
			{title: '终端地址', field: 'unitaddress',width:'100px'}, 
			{title: '终端型号', field: 'unittype', width:'100px'},
...
function searchUnit(){
	...
	var unitaddr = $('#unitaddr').val();
	var unitname = $('#unitname').val();
	var oldver = $('#oldver').val();
	$('#unitTbl').datagrid('load', {
		id: nodeid,
		type: nodetype,
		unitaddr: unitaddr,
		unitname: unitname,
...
function saveScheme(){
	...
	var nodes = $('#unitTbl').datagrid('getSelections');
    ...
    for(var i=0;i<nodes.length;i++){
        data.push({'unitaddress': nodes[i].unitaddress, 'oldver': nodes[i].softwareversion,
        	'newver': newversion})
    }
    $.ajax({
        url:'${pageContext.request.contextPath}/upgradeScheme/addScheme?Math.random()',
        type:'post',
        data:{
