...
<div class="easyui-layout" style="width:100%;height:100%;overflow: hidden">
	...
	<div id="toolbar">
		...
		<!-- 导出 -->
		<form id="exfm"
			action='${pageContext.request.contextPath}/mbBlueBreaker/exportBlueBreakerToExcel'
			method="get">
...
<div id="dlg" class="easyui-dialog"
		...
		<form id="fm" class="easyui-form" method="POST">
			<table cellspacing="8">
			 <tr class="tableTr"> 
                    <td><input class="easyui-textbox" type="text" style="width:300px" id="breakerName" name="breakerName" data-options="label:'断路器名称',required:true"></input></td>
                    <td><input class="easyui-textbox" type="text" style="width:300px" id="breakerCode" name="breakerCode" data-options="label:'断路器编号',required:true"></input></td>
                </tr>
                  <tr class="tableTr"> 
                <td >
                  ...
                  <td><select class="easyui-combobox"
									...
									onSelect: function(node) {
									  var url = '${pageContext.request.contextPath}/mbAmmeter/getAmmeterByMeasurefile?measureId='+node.MeasureId
								    $('#ammeterIds').combobox('reload', url);
               ...
               <tr>
                 <td><select class="easyui-combobox"
									url="${pageContext.request.contextPath}/mbAmmeter/getAmmeterByMeasurefile?measureId=0"
									 id="ammeterIds" style="width: 300px;" data-options="label:'关联电表',required:true,
									 editable:false,valueField:'id', textField:'ammeterName',
									onSelect: function(node) {
								    	$('#ammeterCode').val(node.id);
								    	$('#ammeterIds').combobox('setValue','');
								    }">
								</select>
								<input type="hidden" name="ammeterCode" id="ammeterCode"/>
				  </td>		
				<td><input class="easyui-textbox" type="text" style="width:300px" id="breakerType" name="breakerType" data-options="label:'型号'"></input></td>
               </tr>
                <tr class="tableTr"> 
...
<script type="text/javascript">	
	...
	$(function(){  
		...
		$('#dg').datagrid(
				{
					url : '${pageContext.request.contextPath}/mbBlueBreaker/getAllBlueBreaker?Math.random()',
...
function doSearch() {
	...
	var url="${pageContext.request.contextPath}/mbBlueBreaker/queryBlueBreaker";
...
function add() {
	...
	$('#fm').form('clear');
	url = '${pageContext.request.contextPath}/mbBlueBreaker/addBlueBreaker';
}
...
function edit(index){
	...
	if (row){
		...
		$('#ammeterIds').combobox("setValue",row.ammeterName);
		$('#dlg').dialog('open').dialog('setTitle','编辑型号');
		$('#fm').form('load',row);
		url = '${pageContext.request.contextPath}/mbBlueBreaker/editBlueBreaker?id='+row.id;
	}
}
//保存
function save() {
...
function deleteMeasure(){
     ...
     $.messager.confirm('提示', '确认删除?', function(r) {
         if (r) {
             ...
             $.ajax({
            	 type:'POST', 
		         url:'${pageContext.request.contextPath}/mbBlueBreaker/deleteBlueBreaker',  
                 //dataType : 'json',
                 data:searchid,
                 success:function(result){ 
			    	   if(result.code==200){
...
function importMeasurefile() {
	...
	$('#imfm').form('submit',
	{
		url : '${pageContext.request.contextPath}/mbBlueBreaker/importExcel',
		onSubmit : function() {
			return $(this).form('validate');
		},
...
function exportMeasurefile() {
	window.open("${pageContext.request.contextPath}/mbBlueBreaker/exportBlueBreakerToExcel");
}
var node;
function treeClick(treeObj, n){
	console.log(n);
	...
	var gid=n.gid;
	$('#dg').datagrid({
		url: "${pageContext.request.contextPath}/mbBlueBreaker/queryBlueBreakerByTree",
