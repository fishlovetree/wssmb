...
<!-- 终端列表弹出框 -->
<div id="statisticsdlg" class="easyui-dialog" style="width:1000px;height:520px;" closable="false" closed="true" buttons="#statisticsdlg-buttons"
    data-options="">
...
var statisticsChart = echarts.init(statistics,'DarkGray');

var compared = document.getElementById('compared');
var comparedChart = echarts.init(compared,'DarkGray');
...
function loadAlarmRate(){
	var alarmrateurl = "${pageContext.request.contextPath}/earlyWarn/statisticsAlarmRateQT?Math.random()";
	$.ajax({   type : "POST",
		url : alarmrateurl,
		data : {
			id : $("#selectedID").val(),
...
function loadCompared(){
	if($('#equalNumber').combobox('getValue')=="")
		$('#equalNumber').combobox('setValue',5);
	var alarmrateurl = '${pageContext.request.contextPath}/earlyWarn/alarmComparedQT?Math.random()';
	$.ajax({   type : "POST",
		url : alarmrateurl,
		data : {
			id : $("#selectedID").val(),
...
function makeDatagird(queryParams){
	$('#statisticsdg').datagrid({
		url :'${pageContext.request.contextPath}/earlyWarn/statisticsAlarmListQT?Math.random',
		...
		columns: [[ 
			...
			{title: '设备数据', field: 'occurdata', width: '100px',
				formatter:function(value,rowData,rowIndex){
					if(null!=rowData.id && rowData.id!=""){
						...
						return "<a href='#' class='easyui-linkbutton button-default button-view' onclick='queryData(this,&apos;" + id +"&apos;"+","+"&apos;"+dataType+"&apos;"+","+"&apos;"+rowData.alarmtype+"&apos;);'>查看详情</a>";
					}
            	}
			},
			...
			{title: '安装地址', field: 'installationLocation', width: '150px'},        						 				
			{title: '附件', field: 'annexname', width: '100px',
				formatter:function(value,rowData,rowIndex){
