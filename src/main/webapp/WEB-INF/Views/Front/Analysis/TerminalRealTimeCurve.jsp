	...
	<div id="realtimePanel" class="easyui-layout"
		...
		<div data-options="region:'north'"
			...
			<input type="checkbox" value="8" id="gasconcentration" name="real"
				...
				onclick="getData()" /><span class="cbktext">湿度&nbsp;</span>
				<input
					            ...
					            if(d.result==null&&issearch==true){
									...
									var array = parseThreshold("10",1);					          
						            	...
						            	for ( var p in a) {
											...
											if(a[p].oi=='FF22'){ //湿度
												if(a[p].groupIndex=='1'){
													humidness[index] = FormatValue(a[p].data);		
												}
											}
											if(a[p].oi=='FF21'){ 
												if(a[p].groupIndex=='1'){
				...
				for ( var p in a) {
					if (typeof a[p].freezeTime!="undefined"){
						...
						wsfreezetime.push(formatedate);//冻结时间				
							...
							//温度
							wsambienttemperature[index] = [formatedate,FormatValue(a[p].ambienttemperature)];//温度						
							//电压
